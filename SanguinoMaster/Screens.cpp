...
#define SETPOS(x,y,z)           set_position(LongPoint((long)(x),(long)(y),(long)(z)));
#define MOVETO(x,y,z,stepdelay) queue_absolute_point(x,y,z,stepdelay)
...
const uint8_t jog_length_xy[] = {
  (uint8_t)( 0.1*STEPS_PER_MM_XY+0.5),  //  ~0.1 mm
...
const uint16_t jog_length_z[] = {
  (uint16_t)( 0.1*STEPS_PER_MM_Z+0.5),   //  ~0.1 mm
...
const uint32_t JOG_STEP_DELAY_XY = 1e6/(MAX_MM_PER_MIN_XY*STEPS_PER_MM_XY/60);
const uint32_t JOG_STEP_DELAY_Z  = 1e6/(MAX_MM_PER_MIN_Z *STEPS_PER_MM_Z /60);
...
public Screen {
  virtual void draw() {
    ...
    SCREEN_WRITE_P(PSTR("4)Zero All 8)HomeAll"));
  }

  void seek_x_home() {
#if defined(ENDSTOP_X_MIN_POS)
      seek_minimums(1, 0, 0, JOG_STEP_DELAY_XY, 10);
      pause_stepping();
      LongPoint currpos = get_position();
      SETPOS(ENDSTOP_X_MIN_POS*STEPS_PER_MM_XY,currpos.y,currpos.z);
#elif defined(ENDSTOP_X_MAX_POS)
      seek_maximums(1, 0, 0, JOG_STEP_DELAY_XY, 10);
      pause_stepping();
      LongPoint currpos = get_position();
      SETPOS(ENDSTOP_X_MAX_POS*STEPS_PER_MM_XY,currpos.y,currpos.z);
#endif
  }

  void seek_y_home() {
#if defined(ENDSTOP_Y_MIN_POS)
      seek_minimums(0, 1, 0, JOG_STEP_DELAY_XY, 10);
      pause_stepping();
      LongPoint currpos = get_position();
      SETPOS(currpos.x,ENDSTOP_Y_MIN_POS*STEPS_PER_MM_XY,currpos.z);
#elif defined(ENDSTOP_Y_MAX_POS)
      seek_maximums(0, 1, 0, JOG_STEP_DELAY_XY, 10);
      pause_stepping();
      LongPoint currpos = get_position();
      SETPOS(currpos.x,ENDSTOP_Y_MAX_POS*STEPS_PER_MM_XY,currpos.z);
#endif
  }

  void seek_z_home() {
      seek_minimums(0, 0, 1, JOG_STEP_DELAY_Z, 100);
      pause_stepping();
      LongPoint currpos = get_position();
      SETPOS(currpos.x,currpos.y,0);
  }
  ...
  virtual uint8_t handleKey(char c) {
    ...
    case '1':
      SETPOS(0,currpos.y,currpos.z);
      Screen::change((Screen*)&default_screen);
      return 1;

    case '2':
      SETPOS(currpos.x,0,currpos.z);
      Screen::change((Screen*)&default_screen);
      return 1;
    ...
    case '5':
      ...
      SCREEN_WRITE_P(PSTR("\nSeeking X Axis Home"));
      seek_x_home();
      MOVETO(0, currpos.y, currpos.z, JOG_STEP_DELAY_XY);
      Screen::change((Screen*)&default_screen);
      return 1;
    ...
    case '6':
      ...
      SCREEN_WRITE_P(PSTR("\nSeeking Y Axis Home"));
      seek_y_home();
      MOVETO(currpos.x, 0, currpos.z, JOG_STEP_DELAY_XY);
      Screen::change((Screen*)&default_screen);
      return 1;
    ...
    case '7':
      ...
      SCREEN_WRITE_P(PSTR("\nSeeking Z Axis Home "));
      seek_z_home();
      MOVETO(currpos.x, currpos.y, (long)(Z_AXIS_REST_POS*STEPS_PER_MM_Z), JOG_STEP_DELAY_Z);
      Screen::change((Screen*)&default_screen);
      return 1;
    ...
    case '8':
      ...
      seek_x_home();
      seek_y_home();
      MOVETO(0, 0, (long)(Z_AXIS_REST_POS*STEPS_PER_MM_Z), JOG_STEP_DELAY_XY);
      Screen::change((Screen*)&default_screen);
      return 1;
...
public Screen {
  ...
  virtual uint8_t handleKey(char c) {
    ...
    case '2':
      ...
      send_tool_simple_command_with_word(HEATED_PLATFORM_TOOL, SLAVE_CMD_SET_PLATFORM_TEMP, plat_targ_temp);
      Screen::change((Screen*)&default_screen);
      return 1;
    case '3':
...
public:
  ...
  virtual uint8_t handleKey(char c) {
    ...
    case KEY_YMINUS:
      ...
      point_buffer_clear();
      MOVETO(pos_target_x, pos_target_y, pos_target_z, JOG_STEP_DELAY_XY);
      return 1;
    case KEY_ZMINUS:
      // Z-
      pos_target_z -= jog_length_z[jog_units];
      point_buffer_clear();
      MOVETO(pos_target_x, pos_target_y, pos_target_z, JOG_STEP_DELAY_Z);
      return 1;
    case KEY_EPLUS:
      // Extruder Forwards
